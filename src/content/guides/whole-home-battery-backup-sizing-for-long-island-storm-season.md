...
Financing changes which tier makes sense in practice. A homeowner comparing a $16,000 single-unit system against a $34,000 mid-tier stack on a 10-year loan is really comparing roughly $150/month against roughly $320/month before incentives reduce the principal — a gap that often matters more than the raw kWh difference when a family is deciding how much backup capacity to buy. Both the cash price and the financed monthly number are worth putting on the table during the load-audit conversation, because the "right" tier on paper isn't always the one that fits the household budget for the next decade.

## Sizing Your System the Right Way

The most common sizing mistake on Long Island isn't picking the wrong brand — it's skipping the load audit and buying a battery sized to a national-average household instead of to the actual panel schedule. A [battery storage](/services/battery-storage/) quote should start from a load list off your panel schedule, not a square-footage rule of thumb, and then be matched against how many storm days you want covered without leaning entirely on solar recharge.

If you're pairing storage with a new array, that load audit happens alongside the [residential solar installation](/services/residential-solar-installation/) site assessment, since your production capacity directly affects how many cloudy days a given battery bank can stretch through. [Our install process](/process/) walks through the PSEG-LI interconnection application and Town of Huntington permitting timeline once you've settled on a tier. And to see how these sizing decisions play out across Long Island home types, the [Long Island example system builds](/case-studies/) page models before-and-after load figures for Nassau and Suffolk housing stock.
